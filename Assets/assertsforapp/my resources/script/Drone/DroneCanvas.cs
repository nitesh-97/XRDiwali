...
public class DroneCanvas : MonoBehaviour {
	...
	void Update()
	{
		LButton = transform.position;
		RButton = transform.position;
		ForwardBackward = Drone.GetComponent<DroneControl> ().ForwardBackward;
		Tilt = Drone.GetComponent<DroneControl> ().Tilt;
		FlyLeftRight = Drone.GetComponent<DroneControl> ().FlyLeftRight;
		UpDown = Drone.GetComponent<DroneControl>().UpDown;
